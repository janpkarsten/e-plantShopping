...
const CartItem = ({ onContinueShopping }) => {
  ...
  const calculateTotalAmount = () => {
    let totalcosts = 0;
    cart.forEach((item) => {
      totalcosts += parseInt(item.cost.substring(1)) * item.quantity;
    });
    return totalcosts
  };
  ...
  const handleIncrement = (item) => {
    if (item.quantity > 0) {
      const newquantity = {"name": item.name, "quantity": item.quantity + 1}
  ...
  const calculateTotalCost = (item) => {
    const costs = parseInt(item.cost.substring(1)) * item.quantity;
    return costs
  };
